...
describe('Root Layout Metadata', () => {
  it('should have metadataBase configured', () => {
    ...
    expect(metadata.metadataBase?.toString()).toBe('https://liamwest.me/')
  })
  ...
  it('should have author information', () => {
    ...
    if (Array.isArray(metadata.authors) && metadata.authors.length > 0) {
      ...
      if (typeof author === 'object' && 'name' in author) {
        ...
        expect(author.url).toBe('https://liamwest.me')
      }
    }
  })
...
describe('Contact Page Metadata', () => {
  ...
  it('should have description', () => {
    ...
    expect(
      contactMetadata.description?.toLowerCase().includes('contact') ||
        contactMetadata.description?.toLowerCase().includes('touch')
    ).toBe(true)
  })
...
describe('SEO Best Practices', () => {
  ...
  it('all OG images should use PNG format with correct dimensions', () => {
    const allMetadata: Metadata[] = [metadata, aboutMetadata, projectsMetadata, contactMetadata]
