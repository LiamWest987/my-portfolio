      ...
      <div className={cn(
        ...
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-border px-6 py-5">
          <div className="flex-1">
            <h2 id="modal-title" className="text-3xl font-bold leading-tight text-card-foreground">{project.title}</h2>
          ...
          <button className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border-none bg-transparent text-muted-foreground transition-all hover:bg-secondary hover:text-foreground" aria-label="Close modal" onClick={onClose}>
        ...
        <div className="flex-1 overflow-y-auto p-6 [&::-webkit-scrollbar-thumb:hover]:bg-foreground [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-muted-foreground [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-secondary [&::-webkit-scrollbar]:w-2">
          {allImages.length > 0 && (
            <div className="relative mb-6 w-full overflow-hidden rounded-lg">
              {hasMultipleImages && (
                <button className="absolute left-4 top-1/2 z-10 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border-none bg-black/60 text-white opacity-70 backdrop-blur-sm transition-all hover:scale-110 hover:bg-black/80 hover:opacity-100" aria-label="Previous image" onClick={handlePrevImage}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" focusable="false">
                ...
                {allImages.map((img, index) => (
                  <img
                    ...
                    className={cn(
                      ...
                      index === currentImageIndex ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'
                    )}
                    data-index={index}
              ...
              {hasMultipleImages && (
                <>
                  <button className="absolute right-4 top-1/2 z-10 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border-none bg-black/60 text-white opacity-70 backdrop-blur-sm transition-all hover:scale-110 hover:bg-black/80 hover:opacity-100" aria-label="Next image" onClick={handleNextImage}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" focusable="false">
          ...
          {(project.overview || project.longDescription) && (
            <div className="mb-6">
              ...
              <p className="m-0 text-base leading-relaxed text-muted-foreground" dangerouslySetInnerHTML={{ __html: project.overview || project.longDescription || '' }} />
            </div>
          )}
          ...
          {project.challenges && (
            <div className="mb-6">
              <h3 className="mb-4 text-lg font-semibold text-card-foreground">Challenges & Solutions</h3>
              {Array.isArray(project.challenges) ? (
                ...
                <p className="m-0 text-base leading-relaxed text-muted-foreground">{project.challenges}</p>
              )}
            </div>
          )}
          ...
          {project.outcomes && (
            <div className="mb-6">
              <h3 className="mb-4 text-lg font-semibold text-card-foreground">Outcomes & Impact</h3>
              {Array.isArray(project.outcomes) ? (
                ...
                <p className="m-0 text-base leading-relaxed text-muted-foreground">{project.outcomes}</p>
              )}
            </div>
          )}
