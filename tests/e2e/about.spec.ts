...
test.describe('About Page', () => {
  ...
  test.describe('Keyboard Arrow Navigation', () => {
    ...
    test('should wrap around to last tab when pressing ArrowLeft on first tab', async ({
      page,
    }) => {
  ...
  test.describe('Loading States', () => {
    test('should show skeleton loaders while data is loading', async ({ page }) => {
      // Block Sanity API to force skeleton state
      ...
      // Check for skeleton elements - data may load instantly from cache
      const skeletonCount = await page.locator('[class*="skeleton"]').count()
      const hasData = await page.getByText(/Skills & Experience/i).isVisible()

      // Either we see skeletons OR we see loaded data (fast cache hit)
      expect(skeletonCount > 0 || hasData).toBeTruthy()
    })
  ...
  test.describe('Page Content', () => {
    test('should display page title and description', async ({ page }) => {
      ...
      await expect(
        page.locator('#about').getByText(/Engineering student passionate about/i)
      ).toBeVisible()
    })
