...
async function waitForProjectsLoad(page: unknown): Promise<boolean> {
  ...
  await page
    .waitForSelector('[aria-label="Loading projects"]', { state: 'detached', timeout: 10000 })
    .catch(() => {})
...
test.describe('Projects Page', () => {
  ...
  test.describe('Page Load and Initial State', () => {
    ...
    test('should display results info', async ({ page }) => {
      ...
      const resultsStatus = page
        .locator('[role="status"]')
        .filter({ has: page.locator('text=/Showing.*of.*project/') })
  ...
  test.describe('Category Filtering', () => {
    ...
    test('should show all categories when "All Categories" selected', async ({ page }) => {
      ...
      // Find category dropdown by looking for button near the dropdown menu
      const categoryDropdown = page.locator('#category-dropdown-menu').locator('..')
      const categoryButton = categoryDropdown.getByRole('button').first()

      // Open dropdown and select a specific category
      await categoryButton.click()
      ...
      if (itemCount > 1) {
        // Click second category (first is "All Categories")
        await menuItems.nth(1).click()
        ...
        // Now select "All Categories" again - re-find button since text changed
        await categoryDropdown.getByRole('button').first().click()
        await page.waitForTimeout(200)

        await menuItems.first().click() // Click "All Categories"
        await page.waitForTimeout(500) // Wait for filter to reset

        // Verify button shows "All Categories" again
        const buttonText = await categoryDropdown.getByRole('button').first().textContent()
        expect(buttonText).toContain('All Categories')

        // Verify we're back to showing all projects
  ...
  test.describe('Sort Functionality', () => {
    ...
    test('should sort projects by newest first', async ({ page }) => {
      ...
      await page
        .locator('#sort-dropdown-menu [role="menuitem"]')
  ...
  test.describe('Combined Filters', () => {
    ...
    test('should apply all filters together', async ({ page }) => {
      ...
      await expect(
        page.getByRole('status').filter({ has: page.locator('text=/Showing.*of.*project/') })
      ).toBeVisible()
    })
  ...
  test.describe('Empty States', () => {
    test('should show empty state with no search results', async ({ page }) => {
      ...
      await page
        .getByPlaceholder('Search projects, tags, or technologies...')
  ...
  test.describe('Navigation', () => {
    ...
    test('should maintain scroll position when opening modal', async ({ page }) => {
      ...
      // Allow up to 250px difference to account for layout adjustments when modal opens/closes
      expect(scrollDifference).toBeLessThan(250)
    })
  })
})
