...
test.describe('Projects Page Accessibility', () => {
  ...
  test.describe('Filter Dropdowns Accessibility', () => {
    ...
    test('active category is indicated with aria-current', async ({ page }) => {
      ...
      const activeItem = page.locator(
        '#category-dropdown-menu [role="menuitem"][aria-current="true"]'
      )
  ...
  test.describe('Keyboard Navigation', () => {
    test('can tab to search input', async ({ page }) => {
      ...
      while (attempts < maxAttempts) {
        ...
        if (activeElement.type === 'search' || activeElement.placeholder?.includes('Search')) {
    ...
    test('can tab through project cards', async ({ page }) => {
      ...
      const isFocused = await projectCards
        .first()
  ...
  test.describe('Modal Accessibility', () => {
    ...
    test('modal content is scrollable with keyboard', async ({ page }) => {
      ...
      const isDialogOpen = await existingDialog.isVisible().catch(() => false)
      if (isDialogOpen) {
        await page.keyboard.press('Escape')
  ...
  test.describe('Focus Management', () => {
    ...
    test('skip link is present and functional', async ({ page }) => {
      ...
      // Activate with Enter key instead of click (more realistic keyboard navigation)
      await page.keyboard.press('Enter')
      await page.waitForTimeout(200)
